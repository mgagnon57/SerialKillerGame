...
- [ ] Add a way to set a plot's TYPE/zoning on the parcel editor (residential, shopping/commercial, etc.) - alongside the existing household/character notes in ParcelNotes, so undeveloped or authored parcels can be categorized without needing a full generated Place. Also want to record how true to life the building is - whether it's one or two stories, has a basement, etc. - and cover all the different housing types that exist in a town the size of Rossville: apartment, multilevel apartment complex, single level, duplex, etc. — *2026-08-01*
