...
# ==========================================================================
#  THIS TOOL NO LONGER OWNS Content/city.txt, AND WILL NOT OVERWRITE IT.
#
#  It BOOTSTRAPPED that file on 2026-08-02 and has not been touched since. The file
#  has been edited by hand in 44 commits over the three days after: the roads moved
#  onto their real rights of way, fifteen alleys were laid and then moved off the
#  houses they sat on, Chicago Street got its curve, the downtown stopped being
#  Northgate, the school was built, four 1913 trades were blanked, the steam laundry
#  became a laundromat, and one storefront door moved a single tile so the town was
#  not in two walkable pieces.
#
#  None of that is in here. Re-running this would silently replace all of it with a
#  three-day-old bootstrap, and the "Generated by tools/relay-rossville.py" line at
#  the top of city.txt would make that look like the correct thing to do.
#
#  So it is a mechanism rather than a comment, because a comment is what was there
#  and a comment is exactly what somebody in a hurry does not read. --write says you
#  mean it and prints what you are about to lose first.
# ==========================================================================
target = os.path.join(CONTENT, "city.txt")
body = "\n".join(out) + "\n"

if "--write" not in sys.argv:
    existing = ""
    if os.path.exists(target):
        existing = open(target, encoding="utf-8-sig").read()
    print("REFUSING to overwrite %s" % target)
    print("")
    print("  on disk   : %6d lines" % existing.count("\n"))
    print("  would be  : %6d lines" % body.count("\n"))
    print("")
    print("  That file has been hand-edited in 44 commits since this tool wrote it.")
    print("  Everything authored into it - the curved roads, the alleys, the school,")
    print("  the downtown, the 1991 trades - exists ONLY there and would be lost.")
    print("")
    print("  Pass --write if you have read the above and mean it.")
    sys.exit(1)
